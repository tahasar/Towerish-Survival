...
public class AnimationBuilder : Object
{
    ...
    public AnimationBuilder(ScmlPostProcessor.ScmlProcessingInfo info, IDictionary<int, IDictionary<int, Sprite>> folders,
    ...
    public void Build(Animation animation, IDictionary<int, TimeLine> timeLines)
    {
        ...
        var pendingTransforms =
            new Dictionary<string, Transform>(Transforms); //This Dictionary will shrink in size for every transform
        foreach (var key in animation.mainlineKeys)
        {
            ...
            foreach (var sref in key.objectRefs)
            {
                ...
                if (pendingTransforms.TryGetValue(timeLine.name, out sprite))
                {
                    ...
                    SetCurves(sprite, DefaultSprites[timeLine.name], parentTimeline, timeLine, clip, animation,
                        ref defaultZ);
            ...
            foreach (var kvPair in
                     pendingTransforms) //Disable the remaining tansforms if they are sprites and not already disabled
        ...
        if (!ArrayUtility.Contains(Controller.animationClips, clip))
        {
            ...
            if (!ModdedController)
            {
                if (!ProcessingInfo.NewControllers.Contains(Controller) &&
                    !ProcessingInfo.ModifiedControllers.Contains(Controller))
    ...
    private void SetCurves(Transform child, SpatialInfo defaultInfo, List<TimeLineKey> parentTimeline,
        TimeLine timeLine, AnimationClip clip, Animation animation)
    {
        var defZ = 0f;
    ...
    private void SetCurves(Transform child, SpatialInfo defaultInfo, List<TimeLineKey> parentTimeline,
        TimeLine timeLine, AnimationClip clip, Animation animation, ref float defaultZ)
    {
        var childPath = GetPathToChild(child);
        foreach (var kvPair in
                 GetCurves(timeLine, defaultInfo,
                     parentTimeline)) //Makes sure that curves are only added for properties 
            switch (kvPair.Key)
            {
    ...
    private void SetAdditionalCurves(Transform child, MainLineKey[] keys, TimeLine timeLine, AnimationClip clip,
        float defaultZ)
    {
        ...
        if (kfsZ.Count > 0)
        {
            ...
            if (!positionChanged)
            {
                ...
                clip.SetCurve(childPath, typeof(Transform), "localPosition.x",
                    new AnimationCurve(new Keyframe(0f, info.x)));
                clip.SetCurve(childPath, typeof(Transform), "localPosition.y",
                    new AnimationCurve(new Keyframe(0f, info.y)));
            }
        }
    ...
    private void SetKeys(AnimationCurve curve, TimeLine timeLine, Func<SpatialInfo, float> infoValue,
        Animation animation)
    {
        ...
        var lastIndex =
            animation.looping
                ? 0
                : timeLine.keys.Length - 1; //Depending on the loop type, duplicate the first or last frame
    ...
    private void SetKeys(AnimationCurve curve, TimeLine timeLine, ref Sprite[] sprites, Animation animation)
    {
        foreach (var key in timeLine.keys)
        {
            ...
            curve.AddKey(new Keyframe(key.time, GetIndexOrAdd(ref sprites, Folders[info.folder][info.file]), inf,
                inf));
        ...
        curve.AddKey(new Keyframe(animation.length,
            GetIndexOrAdd(ref sprites, Folders[lastInfo.folder][lastInfo.file]), inf, inf));
    }
    ...
    {
        var rv =
            new Dictionary<ChangedValues, AnimationCurve>(); //This method checks every animatable property for changes
        foreach (var key in timeLine.keys)
        {
            ...
            {
                rv[ChangedValues.PositionX] =
                    new AnimationCurve(); //There will be irregular behaviour if curves aren't added for all members  
                rv[ChangedValues.PositionY] =
                    new AnimationCurve(); //in a group, so when one is set, the others have to be set as well
            ...
            if (!rv.ContainsKey(ChangedValues.RotationZ) && (defaultInfo.rotation.z != info.rotation.z ||
                                                             defaultInfo.rotation.w != info.rotation.w))
            {
                rv[ChangedValues.RotationZ] =
                    new AnimationCurve(); //x and y not necessary since the default of 0 is fine
