...
{
    private readonly IList<TextureImporter>
        InvalidImporters =
            new List<TextureImporter>(); //Importers in this list have already been processed and don't need to be processed again

    private readonly ScmlPostProcessor.ScmlProcessingInfo ProcessingInfo;

    public PrefabBuilder(ScmlPostProcessor.ScmlProcessingInfo info)
    {
        ProcessingInfo = info;
    }
    ...
    public bool Build(ScmlObject obj, string scmlPath)
    {
        ...
        var folders =
            new Dictionary<int, IDictionary<int, Sprite>>(); //I find these slightly more useful than Lists because
        ...
        foreach (var entity in obj.entities)
        {
            ...
            {
                instance = (GameObject)PrefabUtility
                    .InstantiatePrefab(prefab); //instantiates the prefab if it does exist
    ...
    [Obsolete("Obsolete")]
    private void TryBuild(Entity entity, GameObject prefab, GameObject instance, string directory,
        string prefabPath, IDictionary<int, IDictionary<int, Sprite>> folders)
    {
        ...
        if (animator.runtimeAnimatorController !=
            null) //The controller we use is hopefully the controller attached to the animator
            controller =
                animator.runtimeAnimatorController as
                    AnimatorController ?? //Or the one that's referenced by an OverrideController
        ...
        if (controller == null)
        {
            ...
            controller =
                (AnimatorController)AssetDatabase.LoadAssetAtPath(controllerPath, typeof(AnimatorController));
            if (controller == null)
            {
                controller =
                    AnimatorController
                        .CreateAnimatorControllerAtPath(controllerPath); //Or create a new one if it doesn't exist.
        ...
        var transforms =
            new Dictionary<string, Transform>(); //All of the bones and sprites, identified by TimeLine.name, because those are truly unique
        transforms["rootTransform"] =
            instance.transform; //The root GameObject needs to be part of this hierarchy as well
        var defaultBones =
            new Dictionary<string, SpatialInfo>(); //These are basically the object states on the first frame of the first animation
        var defaultSprites =
            new Dictionary<string, SpriteInfo>(); //They are used as control values in determining whether something has changed
        ...
        foreach (var animation in entity.animations)
        {
            ...
            foreach (var timeLine in
                     animation
                         .timelines) //TimeLines hold all the critical data such as positioning and graphics used
            ...
            foreach (var key in animation.mainlineKeys)
            {
...

                {
                    ...
                    if (!transforms.ContainsKey(timeLine.name))
                    {
                        ...
                        if (parents.ContainsKey(bone.parent))
                        {
                            ...
                            var spatialInfo = defaultBones[timeLine.name] =
                                ArrayUtility.Find(timeLine.keys, x => x.id == bone.key).info;
                ...
                foreach (var oref in key.objectRefs)
                {
                    ...
                    if (!transforms.ContainsKey(timeLine.name))
                    {
                        ...
                        var swapper =
                            child.GetComponent<
                                TextureController>(); //Destroy the Sprite Swapper, we'll make a new one later
                        ...
                        var spriteInfo = defaultSprites[timeLine.name] =
                            (SpriteInfo)ArrayUtility.Find(timeLine.keys, x => x.id == 0).info;
                        ...
                        child.localPosition =
                            new Vector3(spriteInfo.x, spriteInfo.y,
                                oref.z_index); //Z-index helps determine draw order
                        child.localEulerAngles =
                            new Vector3(0f, 0f,
                                spriteInfo.angle); //The reason I don't use layers or layer orders is because
                        child.localScale =
                            new Vector3(spriteInfo.scale_x, spriteInfo.scale_y,
                                1f); //There tend to be a LOT of body parts, it's better to treat
                        ...
                        if (!firstAnim)
                            child.gameObject
                                .SetActive(
                                    false); //Disable the GameObject if this isn't the first frame of the first animation
                    }
                }
            ...
            try
            {
                animBuilder.Build(animation,
                    timeLines); //Builds the currently processed AnimationClip, see AnimationBuilder for more info
            }
            catch (Exception e)
            {
                Debug.LogErrorFormat("Unable to build animation '{0}' for '{1}', reason: {2}", animation.name,
                    entity.name, e);
            }
        }
    ...
    private Sprite GetSpriteAtPath(string path, File file, ref bool success)
    {
        ...
        if (importer != null)
        {
            ...
            if ((importer.textureType != TextureImporterType.Sprite || importer.spritePivot.x != file.pivot_x
                                                                    ...
                                                                    (ScmlImportOptions.options != null &&
                                                                     importer.spritePixelsPerUnit !=
                                                                     ScmlImportOptions.options.pixelsPerUnit)) &&
                !InvalidImporters.Contains(importer))
            {
                ...
                settings.ApplyTextureType(TextureImporterType.Sprite,
                    true); //after all the textures have been processed
