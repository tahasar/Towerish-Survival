...
public class Animation : ScmlElement
{
        ...
        looping
    {
        get;
        set;
    } // enum : NO_LOOPING,LOOPING //Dengar.NOTE: the actual values are true and false, so it's a bool
...
public class MainLineKey : ScmlElement
{
    ...
    public float time
    {
        ...
        get => _time;
        set => _time =
            value * 0.001f; //Dengar.NOTE: In Unity, it is measured in seconds instead, so we need to translate that
    }
...
public class Ref : ScmlElement
{
    ...
    public float z_index
    {
        ...
        set => z = value *
                   -0.001f; //body parts to work with. This way the order in layer is reserved for entire Spriter entities
    }
}
...
public class TimeLine : ScmlElement
{
        ...
        objectType
    {
        get;
        set;
    } // enum : SPRITE,BONE,BOX,POINT,SOUND,ENTITY,VARIABLE //Dengar.NOTE (except not in all caps)
...
public class TimeLineKey : ScmlElement
{
    ...
    [XmlAttribute]
    public CurveType
        curve_type { get; set; } // enum : INSTANT,LINEAR,QUADRATIC,CUBIC //Dengar.NOTE (again, no caps)
    ...
    [XmlElement("bone", typeof(SpatialInfo))]
    [XmlElement("object", typeof(SpriteInfo))]
    ...
    public float x
    {
        ...
        set
        {
            if (ScmlImportOptions.options != null)
                _x = value *
                     (1f / ScmlImportOptions.options
                         .pixelsPerUnit); // Convert Spriter space into Unity space using pixelsPerUnit
            else
                _x = value * 0.01f;
        }
    }
